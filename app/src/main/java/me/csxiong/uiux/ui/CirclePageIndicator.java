...
public class CirclePageIndicator extends View {
    ...
    private void initAttrs(Context context, AttributeSet set) {
        if (context != null && set != null) {
            TypedArray ta = context.obtainStyledAttributes(set, me.csxiong.library.R.styleable.CirclePageIndicator);
            //总size
            size = ta.getInt(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_size, 3);
            //选中位置
            position = ta.getInt(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_position, 0);
            //单圆半径
            radius = ta.getDimension(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_radius, 10f);
            //非选中颜色
            unSelectColor = ta.getColor(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_unselect_color, 0xffececec);
            //选中颜色
            selectColor = ta.getColor(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_select_color, Color.WHITE);
            //间距比例
            ratio = ta.getFloat(me.csxiong.library.R.styleable.CirclePageIndicator_cpi_ratio, 2f);
            ta.recycle();
        }
    }
